...
/// @warning it works as a gradient effect not a diffusion effect
/// @bug after some time, some pixels are very different from their neighbors
...
bool RainbowDiffusionEffect::Update(uint32_t current_time)
{
    ...
    if((mutation_possibility < mutation_rate * UINT32_MAX) && (elapsed_time >= mutation_interval)) {
        ...
        pixel_index = get_rand_32() % led_strip->Length(); // Random pixel index
    ...
    rainbow_strip[pixel_index] = (rainbow_strip[pixel_index] + 1) % 360; // Increment the color for diffusion effect
    updated_rainbow_strip[pixel_index] = rainbow_strip[pixel_index]; // Update the color in the updated array
    // If a mutation occurred, update the pixel index
    led_strip->SetPixel(pixel_index, Rainbow360[rainbow_strip[pixel_index]]);
    //last_mutation_time = current_time;
    return_value = true; // A mutation occurred
    ...
    for (int i = 0; i < led_strip->Length(); i++) {
        uint8_t left_index = (i == 0) ? led_strip->Length() - 1 : i - 1;
        uint8_t right_index = (i == led_strip->Length() - 1) ? 0 : i + 1;

        uint16_t left_color = rainbow_strip[left_index];
        uint16_t current_color = rainbow_strip[i];
        uint16_t right_color = rainbow_strip[right_index];
        ...
        */
        if(circular_distance(current_color, left_color) >= min_diffusion_diff) {
            updated_rainbow_strip[left_index] = (updated_rainbow_strip[left_index] + 1) % 360;
        }
        if(circular_distance(current_color, right_color) >= min_diffusion_diff) {
            updated_rainbow_strip[right_index] = (updated_rainbow_strip[right_index] + 1) % 360;
        }
    }
